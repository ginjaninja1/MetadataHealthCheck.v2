...
2. **RESOLVED 2026-07-12 (was open) — `SearchArtist` is the real Stage 1 (C1) call, not unrelated dead code.** It already exists in `FixtureMusicBrainzApiClient.cs` with real fixture data for every relevant SmokeTest case (Sarah Vaughan X/Y, Gus Black, Queen, Florence + the Machine, Del Serino) and its own code comment already anticipated exactly this use ("reserved for a future name/alias-based candidate generation path that doesn't exist yet"). **Decision: wire it in as Strategy B's Stage 1 entry point; do not delete or rewrite it as a separate new method.** Existing fixture data stays as-is.
3. **`Resolvers/MusicBrainz/Strategies/SoftBucketStrategy.cs` — the actual rework, now fully specified as a two-stage flow (V2Specification.md §5.3, updated 2026-07-12):**
   a. **Stage 1 (admission gate, one-time per artist, zero LLR contribution):** call `SearchArtist(source.DisplayName)` (§7.2 C1) to get a candidate-artist pool with names + aliases. Normalize both the Emby name and each candidate's name/every alias using a new configurable replacement table (`ScoringConfig.CandidateGeneration.NameNormalizationRules` — seed list drafted and confirmed 2026-07-12: strip leading "The", fold `&`/`and`/`+`, strip apostrophes, strip `feat`/`featuring`/`ft`/`vs`/`with` suffixes, strip punctuation, fold case, collapse whitespace, normalize diacritics). Compute Levenshtein distance on the normalized remainder; name and every alias weighted **equally** here. Admit if the best distance across name-or-any-alias clears `ScoringConfig.CandidateGeneration.ArtistCandidateMaxEditDistance` (new config value — **RESOLVED, was open sub-decision:** this is our own edit-distance calc, not MB's own relevance `score`; see item 4 below for why both still exist).
   b. **Stage 2 (recording-level corroboration, per track, real evidence):** for each admitted candidate, confirm via the existing `RecordingLookup` (already built for evidence collection) — **reuse this class, don't build a second parallel recording-lookup implementation.** A `/recording` result matching the candidate's primary name counts at `ScoringConfig.NameMatchWeight` (default 1.0); matching only via an alias counts at `ScoringConfig.AliasMatchWeight` (default 0.9) — both multiply whatever Corroboration Tier value (§6.3) that lookup would otherwise contribute. `NameDistanceEvidenceCollector` reports which one applied (`MatchedViaAlias: true/false`) as part of its existing job of rejecting matches too poor to trust at all.
   c. Naming: is `SoftBucketStrategy.cs`'s name/class still appropriate given it's now a two-stage artist-search-first flow rather than a "soft," broad recording fallback? Still an open naming question, not decided — flagging rather than renaming unilaterally.
4. **RESOLVED 2026-07-12 (was open) — `ArtistCandidateMinScore` does NOT need implementation right now, and is not the admission bar.** It stays in `ScoringConfig` as MusicBrainz's own returned Lucene relevance `score` (confirmed real and present in the actual C1 response, 0–100 — an earlier assumption that MB doesn't provide a numeric score was itself wrong, corrected 2026-07-12), defaulted to **0** (inert/no-op today). The real admission bar is the new, separate `ArtistCandidateMaxEditDistance` + `NameNormalizationRules` from item 3a above. `ArtistCandidateMinScore` is retained, not removed, in case future tuning against real resolution volume shows MB's own score adds value alone or in combination.
5. **RETIRED 2026-07-12, do NOT build `AliasEvidenceCollector` as previously scoped.** §6.1's old "Alias match, alone (+0.3)" and "Alias + recording match (joint feature, +1.8)" evidence types are retired from the spec entirely — they pre-dated the current two-stage model (item 3 above) and no longer describe how the engine actually works. There is no standalone alias-evidence collector to build; alias-vs-name match quality is handled entirely by the `NameMatchWeight`/`AliasMatchWeight` multiplier in Stage 2 (item 3b), applied by whatever collector already produces Corroboration Tier evidence (`CorroborationTierEvidenceCollector`) — not a separate collector class.
...
Coding checklist: widen EmbyTrackCredit to the full per-track field set (confirmed 2026-07-12; item 1 + 7 DONE and tested same day; items 2-6 still NOT IMPLEMENTED)
...
1. **DONE, tested 2026-07-12 — `Sources/Emby/EmbyArtist.cs` widened.** Added a new `EmbyCreditedName` class (`{Name, EmbyId, ProviderIds}`) and widened `EmbyTrackCredit` with `AlbumArtists`/`Artists`/`Composers` (each `List<EmbyCreditedName>`) and `Duration` (`TimeSpan?`), matching §8.2's already-specified E2 yield. Fields populated uniformly on every observation regardless of tier, per the earlier clarification — not gated per-tier; a null/empty `Composers` list on an AlbumArtist-tier track is a real fact, not a gap. Purely additive — confirmed via all 33 SmokeTest assertions still passing unchanged after the change.
...
5. **NOT YET DONE — Duration as a real evidence signal:** `Duration` now exists on `EmbyTrackCredit` (step 1) but nothing reads it yet. It's real data MusicBrainz's own recording search already returns (`length` in ms — confirmed against real data, 2026-07-12) and can feed `CorroborationTierEvidenceCollector` as a large-distance elimination/sanity check (§6.1's already-logged residual-risk note names this specifically) — not as a positive corroboration requiring exact match, since real releases of the same recording can differ by a few seconds in mastering (confirmed: the real "Autumn Leaves" data showed a ~5 second spread across genuine releases of the same recording).
6. **NOT YET DONE — unblocking `EmbyArtistObservationUnitProvider.cs`'s own documented gap:** rules 2 (single-credit-tracks-first) and 4 (shorter-albums-first) of §5.5.1's four distance-seeking rules are still unimplemented. The model can now carry what rule 2 needs (step 1), but `EmbyArtistObservationUnitProvider.cs`'s own ordering logic hasn't been updated to use it yet. Rule 4 still needs more than this checklist supplies alone (a true per-album track count).
